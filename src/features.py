...
16 weeks before announcement into a single row of engineered features.
 
Features:
    1. revision_slope          - linear trend of consensus over 16 weeks
    2. revision_acceleration   - second derivative (is slope speeding up?)
    3. direction_changes       - how many times did consensus reverse direction?
    4. final_vs_initial        - consensus at announcement minus consensus 8 weeks prior
...
Outputs:
    - data/processed/features.parquet   (one row per ticker-quarter)
    - logs/features.log
"""
...
def compute_features(group:pd.DataFrame) -> dict:
    """Compute features for a single ticker-quarter"""
    ...
    if n>=2:
        ...
        target_date = last_date - pd.Timedelta(weeks=8)
        idx = (group["date"] - target_date).abs().idxmin()
        initial_consensus = group.loc[idx, "consensus"]
...
def build_features(estimate:pd.DataFrame) -> pd.DataFrame:
    """Group by ticker-quarter and apply compute_features."""
    log.info("Computing revision path features...")

    cq = estimate[estimate["period"] == "Current Quarter"].copy()
    log.info(f"  Current Quarter snapshots: {len(cq):,} rows")
    log.info(f"  Unique ticker-quarters   : {cq.groupby(['act_symbol', 'period_end_date']).ngroups:,}")
    ...
    features_df = features_df[["act_symbol", "period_end_date",
...
def validate_features(features: pd.DataFrame, panel: pd.DataFrame) -> None:
    ...
    feature_keys = set(zip(features["act_symbol"], features["period_end_date"]))
